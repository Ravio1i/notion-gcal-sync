...
def test_get_name_invalid():
    column = "Name"
    properties = {}
    assert NotionEvent.get_name(properties, column) == ""


def test_get_last_edited_time():
    column = "Last Edited Time"
    time = Time(timezone_name="Europe/Berlin")
...
@pytest.mark.parametrize(
    ...
    [
        (
            "2021-10-01T11:37:00.000Z",
            None,
            ("2021-10-01T11:37:00.000Z", "2021-10-01T11:37:00.000Z"),
        ),
        (
            "2021-10-01T11:37:00.000Z",
            "2021-10-01T13:37:00.000Z",
            ("2021-10-01T11:37:00.000Z", "2021-10-01T13:37:00.000Z"),
        ),
...
def test_get_time(test_start, test_end, expected):
    column = "Date"
    properties = {column: {"date": {"start": test_start, "end": test_end}}}
...
def test_get_text():
    column = "Description"
    properties = {column: {"rich_text": [{"text": {"content": "This is a text"}}]}}
...
def test_get_url():
    column = "Url"
    properties = {column: {"url": "https://google.com"}}
    assert NotionEvent.get_url(properties, column) == "https://google.com"


def test_get_select():
    column = "Select"
    properties = {column: {"select": {"name": "Selection1"}}}
...
def test_get_multiselect():
    column = "MultiSelect"
    properties = {column: {"multi_select": [{"name": "Selection1"}, {"name": "Selection2"}]}}
...
def test_get_checkbox(test_input, expected):
    column = "Checkbox"
    properties = {column: {"checkbox": test_input}}
