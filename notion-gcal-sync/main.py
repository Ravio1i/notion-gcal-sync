...
def update_gcal_link(notion_client: NotionClient, notion_event: NotionEvent, gcal_gcal_page_url: str):
    if gcal_gcal_page_url != notion_event.gcal_page_url.replace('&ctz=' + notion_client.cfg.time.timezone_name, ''):
        logging.info('- Updating gcal page url for {} in Notion'.format(notion_event.name))
...
def create_notion_events(df: pd.DataFrame, gcal_client: GCalClient, notion_client: NotionClient, gcal_df: pd.DataFrame,
    ...
    gcal_only_df.columns = [x.replace('_gcal', '') if any(k in x for k in gcal_only_df.columns) else x for x in
                            gcal_only_df]
...
def update_events(df: pd.DataFrame, gcal_client: GCalClient, gcal_df: pd.DataFrame, gcal_specific_columns: list,
    ...
    notion_values_df = notion_values_df.loc[:, notion_df.columns]
    ...
    for idx, el in diff_df.iterrows():
        ...
        if notion_last_updated < gcal_last_updated:
            ...
            logging.info('- Updating event {} in GCal'.format(gcal_updates['name']))
            notion_event = NotionEvent(**gcal_updates.drop(gcal_specific_columns).to_dict(), notion_id=notion_updates['notion_id'],
...
def main():
    ...
    ###########################################################################
    logging.info('Fetching events from gcal')
    ...
    if not notion_df.empty and not gcal_df.empty:
        df = notion_df.merge(gcal_df, on='gcal_event_id', how='outer', indicator=True,
                             suffixes=('_notion', '_gcal')).drop_duplicates()
